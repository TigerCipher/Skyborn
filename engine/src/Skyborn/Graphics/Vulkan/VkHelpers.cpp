...
//
// File Name: VkHelpers.cpp
// Date File Created: 04/24/2023
// Author: Matt
//
// ------------------------------------------------------------------------------
...
{
const char* get_result_string(VkResult result, bool get_extended)
{
    ...
    case VK_ERROR_FRAGMENTED_POOL:
        return !get_extended
                 ...
                 : "VK_ERROR_FRAGMENTED_POOL A pool allocation has failed due to fragmentation of the pool's memory. "
                   "This must only be returned if no attempt to allocate host or device memory was made to accommodate "
                   "the new allocation. This should be returned in preference to VK_ERROR_OUT_OF_POOL_MEMORY, but only "
                   "if the implementation is certain that the pool allocation failure was due to fragmentation.";
...
bool was_success(VkResult result)
{
    // From: https://www.khronos.org/registry/vulkan/specs/1.2-extensions/man/html/VkResult.html
    switch (result)
    {
