...
#include "Platform.h"
    ...
    #include "Skyborn/Core/Event.h"
    #include "Skyborn/Debug/Logger.h"
    #include "Skyborn/Core/Input.h"
    #include "Skyborn/Graphics/Vulkan/VulkanCommon.h"
...
input::key::code split_key_codes(bool pressed, i32 in_left, i32 in_right, input::key::code out_left, input::key::code out_right)
{
    input::key::code key{};
    if (pressed)
    {
...
LRESULT CALLBACK process_messages(HWND hwnd, u32 msg, WPARAM wparam, LPARAM lparam)
{
    ...
    case WM_SYSKEYUP:
    {
        ...
        if (wparam == VK_MENU)
        {
            k = split_key_codes(pressed, VK_LMENU, VK_RMENU, input::key::lalt, input::key::ralt);
        } else if (wparam == VK_SHIFT)
        {
            k = split_key_codes(pressed, VK_LSHIFT, VK_RSHIFT, input::key::lshift, input::key::rshift);
        } else if (wparam == VK_CONTROL)
        {
            k = split_key_codes(pressed, VK_LCONTROL, VK_RCONTROL, input::key::lcontrol, input::key::rcontrol);
        }
